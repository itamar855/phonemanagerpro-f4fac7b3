...
| 23/09/2026 | `Bugfix / Feature` | `Equipe.tsx`, Edge Functions (`create-user`, `delete-user`) | **Correção de Criação e Exclusão de Membros de Equipe (Multi-loja):** (1) `create-user` agora reaproveita e restaura contas pré-existentes no Supabase Auth, atribui permissões padrão conforme o cargo (`defaultPermissions`), sincroniza `member_stores` e trata erros reais retornando mensagens legíveis para a interface; (2) Criada e implantada a Edge Function `delete-user` para exclusão administrativa com `service_role`, tratando integridade referencial com histórico contábil (vendas/OS) e desativação segura no Auth sem corromper transações passadas; (3) `Equipe.tsx` atualizado para exibir as mensagens de erro detalhadas da Edge Function em vez do erro genérico `non-2xx status code`. |
| 23/09/2026 | `Feature` | `AppLayout.tsx`, `Equipe.tsx`, `ChangePasswordModal.tsx`, Edge Function (`admin-change-password`) | **Gestão Completa de Senhas (Admin e Usuário):** (1) Adicionado componente modal `ChangePasswordModal` integrado no `AppLayout` (barra lateral desktop e gaveta inferior mobile) permitindo que qualquer colaborador autenticado altere sua própria senha com validação em tempo real via `supabase.auth.updateUser`; (2) Criada e implantada a Edge Function `admin-change-password` com `service_role` e integrada na aba "Editar Membro" em `Equipe.tsx`, permitindo que administradores redefinam a senha de qualquer usuário da equipe diretamente pelo painel. |
| 24/09/2026 | `Feature` | `cnpjService.ts`, `fiscalService.ts`, `ConfiguracoesFiscais.tsx`, `NotasFiscais.tsx`, `DetalhesVendaModal.tsx`, `AppLayout.tsx`, `Equipe.tsx` | **Módulo Completo de Emissão de Nota Fiscal Eletrônica Oficial (NF-e / NFC-e / Trade-In):** (1) `cnpjService.ts`: Auto-preenchimento cadastral completo a partir do CNPJ com validação dos 2 dígitos verificadores e busca via BrasilAPI/Receita Federal (razão social, nome fantasia, endereço com código IBGE do município, CNAE fiscal e regime do Simples Nacional); (2) `fiscalService.ts`: Motor de cálculo de Chave de Acesso SEFAZ de 44 dígitos com Dígito Verificador via Módulo 11 (pesos de 2 a 9), suporte a ambientes de Homologação e Produção, controle de numeração sequencial e trava de cancelamento legal perante a SEFAZ (janela máxima de 24 horas e justificativa mínima de 15 caracteres); (3) `ConfiguracoesFiscais.tsx`: Painel de parametrização fiscal por loja com upload de certificado digital A1 (.pfx) e token CSC; (4) `NotasFiscais.tsx`: Central de notas fiscais com KPIs, filtros, emissão avulsa, emissão de NF-e de Entrada Trade-In (CFOP 1102), visualização e impressão de DANFE oficial em HTML/CSS, download de XML e exportação em lote para a contabilidade; (5) `DetalhesVendaModal.tsx`: Ação rápida de emissão de NFC-e oficial diretamente da tela de detalhes da venda; (6) 15 novos testes unitários adicionados (`fiscal-nfe.test.ts`), totalizando 220 testes passando com 0 erros de compilação. |
