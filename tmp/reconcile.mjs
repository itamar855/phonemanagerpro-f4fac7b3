...
async function main() {
  ...
  for (const tx of transactions) {
    // Only care about sales and accessories
  ...
  for (const m of missing) {
    ...
    // Find an open register for this store
    ...
    if (register) {
      const payload = {
        ...
        payment_method: 'dinheiro', // defaulting, though we could try to parse [MISTO...]
        confirmed: false,
      ...
      else if (m.description.includes('[MISTO')) payload.payment_method = 'misto';
